...
const enterKey = async (index) => {
  ...
  const newTextareaValue = currentText.substring(currentIndex, currentTextarea.value.length);
...
const deleteKey = async (index, event) => {
  ...
  if (event.key === 'Delete') {
    ...
    currentTextarea.value = currentTextareaContentObj.contents + nextTextareaContent;
    currentTextarea.setSelectionRange(currentTextareaLastInedx, currentTextareaLastInedx)
    ...
    if (currentIndex==0) {
      ...
      const currentText = textareaContents[index].contents
      const concatTextareaValue = currentText.substring(0, currentTextarea.value.length);
      ...
      prevTextarea.setSelectionRange(prevTextareaLastInedx, prevTextareaLastInedx)
...
const arrowUpKey = (index) => {  
  ...
  const prevTextarea = textareaRefs.value[index-1];
  prevTextarea.focus();      
  prevTextarea.setSelectionRange(prevTextarea.value.length, prevTextarea.value.length+1)
}
...
const calcHeight = () => {
  ...
  for (var ref of textareaRefs.value) {
    ...
    let lineCount = Math.ceil(textCount / lineLength);
...
<template>
  <div v-for="(_, index) in textareaContents" :key="index">
    ...
    <div style="text-align: center;">
      <img class="image" :src="textareaContents[index].contents" alt="Image" v-if="showImageTag(textareaContents[index].contents)">
    </div>
  </div>
</template>
